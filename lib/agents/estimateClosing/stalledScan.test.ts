...
describe("runEstimateClosingShadowSweepWithTelemetry (P1 Sprint 5)", () => {
  ...
  it("active tenant + zero candidates creates exactly one durable estimate.closing_scan_completed record, and zero agent_run/model_invocation/recommendation/tool_call/approval/outcome", async () => {
    ...
    expect(result.telemetryWriteFailures).toBe(0);
  ...
  it("a telemetry write failure does not re-run the scan, does not trigger a second model call, and does not convert a successful scan into a failure", async () => {
    const reader = new InMemorySequenceWithLeadReader([candidate()]);
    const eventStore = new InMemoryBusinessEventStore();
    const runStore = new InMemoryAgentRunStore();
    const agentStore = await activeAgentStore();
    ...
    if (!result.ok) throw new Error("expected ok");
    expect(result.telemetryWriteFailures).toBe(1);
  ...
  it("a genuine scan failure (candidate read throws) is reported as ok:false with a sanitized error category, and still attempts tenant-scoped failure telemetry for already-known relevant tenants", async () => {
    ...
    expect(result.telemetryWriteFailures).toBe(0); // one relevant tenant, one successful failure-telemetry write
  ...
  it("cannot even resolve the relevant-tenant set — no failure telemetry is fabricated for an unknown tenant list", async () => {
    const eventStore = new InMemoryBusinessEventStore();
    const failingAgentStore = { listActiveByType: vi.fn().mockRejectedValue(new Error("agents table unavailable")) };

    const result = await runEstimateClosingShadowSweepWithTelemetry(
      { reader: new InMemorySequenceWithLeadReader([]), eventStore, now: NOW },
      { eventStore, isEnabled: () => true },
      { agentStore: failingAgentStore, telemetry: { eventStore } }
    );

    expect(result.ok).toBe(false);
    if (result.ok) throw new Error("expected failure");
    expect(result.errorCategory).toBe("unknown");
    expect(result.telemetryWriteFailures).toBeNull();

    // No tenant is known, so nothing could have been written anywhere.
    const allA = await eventStore.listByTenant(TENANT_A, { eventType: "estimate.closing_scan_failed" });
