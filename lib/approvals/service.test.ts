...
describe("approval enforcement (P0.4)", () => {
  it("requestApproval creates a pending approval", async () => {
    ...
    const approval = await requestApproval({ tenantId: "t1", requestedAction: "draft_customer_message.execute" }, store);
  ...
  it("only a tenant owner may approve — staff is refused", async () => {
    const store = new InMemoryApprovalStore();
    const approval = await requestApproval({ tenantId: "t1", requestedAction: "x" }, store);
  ...
  it("an expired approval cannot be approved — it transitions to 'expired' instead", async () => {
    ...
    const approval = await requestApproval(
      { tenantId: "t1", requestedAction: "x", expiresAt: new Date(Date.now() - 1000).toISOString() },
      store
    );
