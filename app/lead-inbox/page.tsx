      ...
      <div className="grid gap-4">
        {sortedLeads.map((lead) => {
          ...
          return (
            <div
              key={lead.id}
              className={`rounded-2xl bg-white shadow-sm ring-1 transition hover:shadow-md ${
                isEmergency ? "border-2 border-red-500 ring-red-200" : "ring-slate-200"
              } ${isLowPriority ? "opacity-55" : "opacity-100"}`}
            >
              <Link href={`/leads/${lead.id}`} className="block p-4">
                <div className="flex flex-wrap items-start justify-between gap-2">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {/* Inline status select — stops click propagation so it doesn't navigate */}
                ...
                <p className="mt-2 text-sm text-slate-600">
                  <span className="font-semibold text-slate-900">Job:</span> {lead.jobDescription}
                </p>
                {lead.internalNotes && (
                  <p className="mt-1 text-sm text-slate-500">
                    <span className="font-semibold text-slate-700">Internal:</span> {lead.internalNotes}
                  </p>
                )}
              </Link>
            </div>
