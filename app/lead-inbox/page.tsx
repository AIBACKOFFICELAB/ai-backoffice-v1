...
export default function LeadInboxPage() {
  const leads = [...plumbingLeads].sort((a, b) => {
            ...
            >
              <div className="flex flex-wrap items-start justify-between gap-2">
                <div>
                  ...
                  <p className="text-sm text-slate-600">{new Date(lead.date).toLocaleString()} • {lead.phone} • {lead.email || "No email provided"}</p>
                </div>
                <div className="flex flex-wrap gap-2">
                  {isNew && <span className="rounded-full bg-blue-100 px-2 py-1 text-xs font-semibold text-blue-700">New Lead</span>}
