...
export default async function LeadDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const lead = plumbingLeads.find((item) => item.id === id);
  if (!lead) notFound();
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
        ...
        <p className="mt-2 text-slate-600">All lead data mapped from the 5 Star Plumbing Google Sheet model.</p>
      </section>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200 space-y-1 text-sm text-slate-700">
          ...
          <p>{lead.phone}</p><p>{lead.email || "No email provided"}</p><p>{lead.serviceAddress}</p>
          <p><span className="font-semibold">Role:</span> {lead.customerRole}</p>
        </div>
        <div className="rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200 space-y-1 text-sm text-slate-700">
          <h2 className="font-semibold text-slate-900">Job & Sales Info</h2>
          ...
          <p><span className="font-semibold">Estimate Amount:</span> ${lead.estimateAmount.toLocaleString()}</p>
          <p><span className="font-semibold">Follow-Up Date:</span> {lead.followUpDate}</p>
          <p><span className="font-semibold">Review Request Status:</span> {lead.reviewRequestStatus}</p>
          <p><span className="font-semibold">Preferred Appointment:</span> {lead.preferredAppointmentTime}</p>
      ...
      <section className="rounded-xl bg-white p-5 shadow-sm ring-1 ring-slate-200">
        ...
        <p className="mt-2 text-sm text-slate-700">{lead.jobDescription}</p>
      </section>
      ...
      <section className="rounded-xl border border-blue-100 bg-blue-50 p-5 shadow-sm">
        ...
        <p className="mt-2 text-sm text-slate-700">{recommendedAction(lead.status, lead.emergency, lead.reviewRequestStatus)}</p>
      </section>
    </div>
  );
}
