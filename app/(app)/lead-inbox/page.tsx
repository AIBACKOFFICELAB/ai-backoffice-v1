...
export default async function LeadInboxPage() {
  ...
  const { leads, source, error } = await getLeads(tenant.tenantId);
  ...
  return (
    <div className="space-y-6">
      ...
      />
      {error ? <p role="alert" className="rounded-card bg-white p-5 text-ink-700">Lead Inbox is temporarily unavailable. Please refresh to try again.</p> : <LeadInboxList leads={leads} readOnly={source !== "supabase"} />}
    </div>
  );
}
